...
import 'package:shoes/app/data/model/shoes_model.dart';

class HomeController extends GetxController with GetTickerProviderStateMixin {
  late AnimationController controllerAnimation;
  late Animation<double> animation;
  RxList<ShoesModel> listData = RxList();
  final RxList<ShoesModel> listShoesInCart = RxList();
  ...
  @override
  void onInit() {
    listData.value = List<ShoesModel>.from(data['shoes']!.map((x) => ShoesModel.fromJson(x)));
    controllerAnimation = AnimationController(
      vsync: this,
      duration: const Duration(seconds: 10), // Thời gian diễn ra animation
    );
  ...
  void caculator() {
    totalPrice.value =
        roundUpToTwoDecimalPlaces(listShoesInCart.fold<double>(0, (sum, s) => sum + (s.price ?? 0) * s.quantity));
  }
  ...
  double roundUpToTwoDecimalPlaces(double value) {
    ...
    if (decimalPart > 0) {
      return (intPart + 1) + (decimalPart / 100);
    } else {
      return value;
    }
  }
